...
import { nowDateIso, normalizePaymentMethod, parseNumber } from '../lib/normalizers.js';
...
export async function registerFinancePayment(payload) {
  return withLock(`finance:${payload.cobrancaId}`, async () => {
    ...
    if (!cobranca) {
      return { ok: false, status: 404, error: 'operation_failed', message: 'Billing entry not found' };
    }
    ...
    if (delta <= 0) {
      return { ok: false, status: 400, error: 'validation_error', message: 'Payment amount must be positive' };
    }
    if (paid + delta > total) {
      return {
        ok: false,
    ...
    const nextStatus = computePaymentStatus({ total, paid: nextPaid, dueDate: cobranca.data_vencimento });
    const patch = {
      valor_pago: nextPaid,
      status: nextStatus,
    ...
    if (!updated) {
      return { ok: false, status: 500, error: 'operation_failed', message: 'Failed to update billing entry' };
    }
