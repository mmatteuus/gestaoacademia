...
import { listAllSheets, readSheetValuesSafe, rowToObject } from './schema.js';
...
export async function getRowById(id, type = 'Alunos') {
  ...
  const match = rows.find((row) => row[idIndex] === String(id));
  return match ? rowToObject(headerRow, match, config.headers) : null;
}
