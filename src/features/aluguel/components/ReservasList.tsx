...
import type { ReservaDetalhada } from '../useAluguelPage';

export function ReservasList({ reservas }: { reservas: Reserva[] }) {
  ...
  return (
    <div className="space-y-3">
      {reservas.map((reserva) => {
        ...
        return (
          <div key={reserva.id} className={cn('rounded-lg border bg-card p-4 transition-colors hover:bg-accent/30', reserva.conflito ? 'border-destructive/50' : 'border-border')}>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="font-medium text-foreground">R$ {reserva.valor.toFixed(2)}</span>
            </div>
            {reserva.conflito && (
